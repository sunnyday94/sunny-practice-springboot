...
public interface IBasicDao<E extends Serializable> {
	...
	/** 分页，返回VO集合,支持hql中带in(?),in时参数必须为数据类型
	 * @param <T>
	 ...
	 * @return 返回VO List
	 */
	<T> ResBean.PageInfo<T> getPages(Class<T> clazzVO, int first, int max, String hql, Object ... values);
	/**分页，返回VO集合 ,支持hql中带in(?),in时参数必须为数据类型
	 * @param clazzVO
	 * @param first
	 * @param max
	 ...
	 * @return
	 */
	<T> ResBean.PageInfo<T> getPages(Class<T> clazzVO,int first, int max, String hql, Map<String, Object> pars);
	 ...
	 * @return
	 */
	ResBean.PageInfo<E> getPages(int first, int max, String hql, Object... paras);
	 ...
	 * @return
	 */
	ResBean.PageInfo<E> getPages(int first, int max, String hql, Map<String, Object> pars);
//	ResBean.PageInfo getPages(Class<E> clazz,int first, int max, String hql, Map<String, Object> pars);
	/**分页直接返回List
	 * @param page
	 * @param pagesize
	 * @param hql
...
//    <T> List<T> entityListSQL(Class<T> t,int first,int max, String sql, Map<String, Object> pars);
